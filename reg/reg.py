...
class GLX:
    ...
    @staticmethod
    def from_xml(element: ET.Element) -> "GLX":
        type_ = element.get("type")
        if type_ is None:
            raise attr_not_found_error("GLX", "type")

        opcode = element.get("opcode")
        if opcode is None:
            raise attr_not_found_error("GLX", "opcode")
...
class Command:
    ...
    def from_xml(element: ET.Element) -> "Command":
        ...
        if proto_element is None:
            raise elm_not_found_error("Command", "proto")
        ...
        return Command(
            ...
            glxs=[GLX.from_xml(glx) for glx in element.findall("glx")],
        )
...
class InterfaceElement:
    type_: Literal["type", "command", "enum"]
    name: str

    @staticmethod
    def from_xml(element: ET.Element) -> "InterfaceElement":
        type_ = element.tag
        if type_ not in ["type", "command", "enum"]:
        ...
        return InterfaceElement(
            type_=cast(Literal["type", "command", "enum"], type_),
            name=name,
        )
...
class Interfaces:
    ...
    def from_xml(element: ET.Element) -> "Interfaces":
        return Interfaces(
            ...
            elements=[InterfaceElement.from_xml(elm) for elm in element],
        )
